...
def attach_pdf(doc, attach_to_doc, event=None):
    fallback_language = frappe.db.get_single_value("System Settings", "language") or "en"
    args = {
        "attach_to_doc": attach_to_doc.doctype,
        "attach_to_name": attach_to_doc.name,
        "doctype": doc.doctype,
        "name": doc.name,
        "title": doc.get_title(),
...
def execute(attach_to_doc, attach_to_name, doctype, name, title, lang=None, show_progress=True):
    """
    ...
    fileurl = save_and_attach(pdf_data, attach_to_doc, attach_to_name, name, title_folder)
...
def save_and_attach(content, to_doctype, to_name, pdf_name, folder):
    """
    Save content to disk and create a File document.
    File document is linked to another document.
    """
    file_name = "{}.pdf".format(pdf_name.replace(" ", "-").replace("/", "-"))
